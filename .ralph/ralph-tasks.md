...
- [x] Epic: Discovery + SODA RAG Integration for Agent Guidance
  ...
  - [x] Add a small “Developer Reference” panel (read-only) that can surface the relevant RAG snippet for the current query context without exposing secrets
  - [x] Ensure Developer Reference panel fetches data via `/api/*` only (no direct file fetches)
  - [x] Acceptance: when running discovery planning, the system demonstrates it can retrieve the relevant RAG chunk for a parameter/endpoint, uses it to build valid queries, and avoids unsupported parameters; SODA endpoint construction follows RAG-defined formats; all calls remain same-origin and zero-cost by default
