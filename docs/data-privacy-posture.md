...
- **Debug raw synthesis**: raw model output is kept in memory only and disabled for address-like topics.
...
- **In-memory debug data** (raw synthesis output) is cleared on tab close or page reload; for address-like topics it is not written.
